...
入队前先完成用户级 Token 配额快速检查、Redis 原子预留和请求脱敏；请求侧命中 `BLOCK`
规则时不投递 RabbitMQ，而是注册 SSE owner 后直接缓存/推送成功 `result` 事件，响应内容为命中规则
replacement 按原文位置拼接后的文本。只有治理继续通过的 `ChatRequest` 会投递到 RabbitMQ。
`ChatRequest` 会携带用户侧 reservation ids；`ChatRabbitConsumer` 消费后不再重复请求配额校验，
只执行流式 Agent 管线，`ProviderTokenBudgetGuardStage` 在模型解析后对 Provider 日/周/月预算做 Redis 原子预留，
随后把 meta、token、工具进度、最终结果或失败直接交给 `SsePushService` 推送给前端。
