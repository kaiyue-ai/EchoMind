...
| 当前对话记忆 | MySQL `echomind_chat_sessions` / `echomind_chat_messages`，按 `userId + sessionId` 保存完整历史，用于前端历史展示 | Redis `echomind:memory:recent:*` 最近 50 条上下文缓存 |
...
flowchart TD
    ...
    W --> X["ChatMemoryPersistConsumer: 写 MySQL / Redis 最近 50 条"]
    X --> R["RabbitMQ: echomind.user-memory.requests"]
    R --> S["echomind-user-memory: Redis 缓冲满 5 轮后批量更新事实层和画像"]
...
`ChatMemoryPersistConsumer` 再写 MySQL 完整历史、Redis 最近 50 条上下文，并把本轮消息交给
`echomind-user-memory`。用户记忆 worker 按用户全局缓冲 5 轮对话后，一次调用 LLM，基于旧画像、
5 轮对话和相关旧事实输出事实层 add/update/delete 以及新版用户画像快照。普通聊天消息不再写入
Redis Stack 向量索引。
      ...
      deepseek:
        ...
        models:
          - name: deepseek-v4-flash
            ...
            default: true
  memory:
    short-term-window: 50       # 模型提示词直接从 Redis 读取的近期消息条数
