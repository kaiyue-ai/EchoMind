...
当前 Docker Compose 部署已内置一条本地演示用观测链路：`echomind-backend` 通过 OpenTelemetry Java Agent
把 Trace 上报给 OpenTelemetry Collector，再转发到 SkyWalking 展示。此链路当前只覆盖后端 Trace，
不包含指标和日志，也不以长期存储为目标。
...
| 当前对话记忆 | MySQL `echomind_chat_sessions` / `echomind_chat_messages`，用于前端历史展示 | Redis `echomind:memory:recent:*` 最近上下文缓存 + Redis Stack 向量索引 |
| 用户长期画像 | Redis Stack `idx:user:memory:vectors`，按 `sessionId` 隔离 | 主应用 Pipeline 同步 KNN 召回 |
| Agent 知识库向量 | Redis Stack Agent 知识库索引 | MySQL 只保留文档、切片正文和元数据 |
...
flowchart TD
    ...
    L --> U["UserMemoryRetrievalStage: 按 sessionId 召回用户长期画像"]
    U --> M["ToolResolutionStage: 解析模型与可用工具"]
    M --> O["ResultAggregationStage: 注册工具函数并调用模型"]
    O --> P["MemoryPersistStage: 写回会话记忆"]
    P --> R["RabbitMQ: echomind.user-memory.requests"]
    R --> S["echomind-user-memory: LLM 提取画像并写 Redis Stack"]
...
当前 DeepSeek Provider 通过 Anthropic-compatible `stream: true` 解析 `text_delta`，OpenAI 兼容 Provider
（OpenAI、阿里云百炼/Qwen、Ollama、vLLM 等）通过 Chat Completions `stream: true` 解析
`choices[].delta.content`；Mock Provider 仍只返回单段模拟结果。
...
| 12 | `UserMemoryRetrievalStage` | 按 `sessionId` 从 Redis Stack 召回用户长期画像并注入 system message |
...
| `echomind-llm` | 动态模型路由，支持 DeepSeek、自研 OpenAI 兼容 Provider 和阿里云百炼 |
| `echomind-memory` | MySQL 完整会话历史供前端展示 + Redis 最近上下文供 LLM 读取 + Redis Stack/百炼向量检索，记忆按 sessionId 隔离 |
| `echomind-user-memory` | RabbitMQ 异步消费聊天事件，LLM 提取用户长期画像并写入 Redis Stack |
...
| otel-collector | 4317 / 4318 | OTLP 接收与转发 |
| skywalking-oap | 11800 / 12800 / 9412 | SkyWalking 查询与 OTLP/Zipkin 兼容接收 |
| skywalking-ui | 8081 | SkyWalking Trace 展示界面 |
...
Docker Compose 路径会自动给后端挂载 OpenTelemetry Java Agent，并把 Trace 通过
OpenTelemetry Collector 转发到 SkyWalking。
...
这是本地演示型链路，只覆盖后端 Trace，不承诺长期保留数据，也不包含指标和日志。
...
# 如果同时验证链路追踪，建议显式拉起观测组件
docker compose up -d skywalking-oap otel-collector skywalking-ui backend
...
2. 调用一次 `POST /api/chat/sync`
3. 在 SkyWalking UI 中搜索 `echomind-backend`

说明：

- 当前方案采用 OpenTelemetry Java Agent 零侵入接入。
- 外部 MCP 的 `java -jar` 子进程会显式剥离 `OTEL_*` 环境变量，避免 trace 混入主后端服务。
- SkyWalking OAP 当前复用 Docker Compose 中的 MySQL 作为本地演示型存储，并通过环境变量覆盖官方默认配置启用 OTLP/Zipkin 查询链路。
