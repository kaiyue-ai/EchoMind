...
当前 Docker Compose 默认部署业务依赖和前后端，不再让后端启动依赖本地链路追踪组件；如需 Trace，
请把 OpenTelemetry Collector / SkyWalking 作为独立观测栈接入。
...
| 当前对话记忆 | MySQL `echomind_chat_sessions` / `echomind_chat_messages`，按 `userId + sessionId` 保存完整历史，用于前端历史展示 | Redis `echomind:memory:recent:*` 最近上下文缓存 + Redis Stack 向量索引 |
| 用户长期画像 | Redis Stack `idx:user:memory:vectors`，按 `userId + sessionId` 隔离 | 主应用 Pipeline 同步 KNN 召回 |
...
flowchart TD
    ...
    L --> U["UserMemoryRetrievalStage: 按 userId + sessionId 召回用户长期画像"]
    ...
    W --> X["ChatMemoryPersistConsumer: 写 MySQL / Redis / 向量"]
    X --> R["RabbitMQ: echomind.user-memory.requests"]
    R --> S["echomind-user-memory: LLM 提取画像并写 Redis Stack"]
    P --> Q["ChatResponse / SSE token"]
```

聊天接口会先从 `Authorization: Bearer ...` 解析当前用户；无 token 的兼容请求归属 `default`
用户。同步请求走 `POST /api/chat/sync`，`ChatController` 创建或复用前端 `sessionId` 后，调用
...
| 12 | `UserMemoryRetrievalStage` | 按 `userId + sessionId` 从 Redis Stack 召回用户长期画像并注入 system message |
...
`ChatMemoryPersistConsumer` 再写 MySQL 完整历史、Redis 最近上下文和 Redis Stack 普通消息向量。
...
对 URL 场景有额外防线：`http/https` 链接会强命中通用 `web-search`，
域名专用工具只在 URL 域名匹配时暴露，例如牛客 MCP 不会处理 CSDN、知乎等非牛客链接。
...
| `echomind-memory` | MySQL 完整会话历史供前端展示 + Redis 最近上下文供 LLM 读取 + Redis Stack/百炼向量检索，普通聊天记忆按 `userId + sessionId` 隔离 |
| `echomind-user-memory` | RabbitMQ 异步消费聊天事件，LLM 提取用户长期画像并按 `userId + sessionId` 写入 Redis Stack |
...
- 默认部署不再启动 OpenTelemetry Collector / SkyWalking，避免观测组件健康状态影响后端启动。
- 如果需要链路追踪，请使用独立观测栈接入，不要把业务服务启动依赖在观测组件健康检查上。
