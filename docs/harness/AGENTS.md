...
- RabbitMQ 用途：只承接 `echomind.chat.requests` 异步聊天请求、`echomind.chat-memory.persist.exchange` 普通聊天记忆分片写入，以及 `echomind.user-memory.requests` 用户长期记忆事件；核心队列失败重试耗尽后进入 DLQ，由主后端归档到 MySQL 并支持受控重放；Agent Team 当前不走 RabbitMQ。
