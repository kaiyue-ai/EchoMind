...
- RabbitMQ 用途：只承接 `echomind.chat.requests` 异步聊天请求、`echomind.chat-memory.persist.exchange` 普通聊天记忆分片写入，以及 `echomind.user-memory.requests` 用户长期记忆事件；核心队列失败重试耗尽后进入 DLQ，由主后端归档到 MySQL 并支持受控重放；Agent Team 当前不走 RabbitMQ，Run 级调度由 `TeamExecutionEngine` 适配本地 `TaskExecutor`。
...
- Agent Team：已演进为 MySQL 黑板驱动的异步 DAG 协作，Planner / PlanReview / TeamControlCenter / AgentSelector / Executor / StepReviewer / MergeAgent / ConflictDetector / PlannerArbitration / GlobalReviewer 通过 Run / Step / Event 交换上下文；Team 当前通过 `TeamExecutionEngine` 调度、本地 `TaskExecutor` 推进，RabbitMQ/Redis DAG 投影属于后续演进。
...
flowchart TB
    ...
    subgraph DataAndInfra["数据与基础设施"]
        ...
        Rabbit["RabbitMQ<br/>chat.requests<br/>chat-memory shards / user-memory<br/>DLQ archive / replay"]
...
- 可靠队列只覆盖聊天请求、普通聊天记忆分片和用户长期记忆事件；在线 SSE token/tool/result/failure 事件仍由消费端直接推送，不进 RabbitMQ 强可靠队列。
...
- Team 后续接 RabbitMQ 时，RabbitMQ 只做可靠命令队列，MySQL 仍保存 Run / Step / Event / Outbox 事实，Redis 只保存可重建 DAG 热投影。Team command 必须经 Outbox 发布，消费者必须用 epoch / iteration / attemptId / CAS 做幂等。
