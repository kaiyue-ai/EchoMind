...
- 记忆系统：MySQL 按 `userId + sessionId` 保存普通聊天完整会话；Redis 按字符预算保存单会话短期上下文；Redis Stack 保存用户长期事实向量和 Agent 知识库向量；Redis Hash 保存用户画像快照。
...
- Redis 是 LLM 最近上下文来源；Redis Stack 是用户长期事实和知识库向量事实来源；Redis 用户画像快照是事实层的压缩结果；LLM 不从 MySQL 读取聊天历史。
