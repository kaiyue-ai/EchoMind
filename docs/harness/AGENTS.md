...
- 对话执行：同步入口 `POST /api/chat/sync` 直接进入 `AgentOrchestrator -> Agent -> ExecutionPipeline`；异步入口 `POST /api/chat` 入队，`GET /api/chat/stream/{requestId}` 订阅 RabbitMQ 消费后的 SSE token/result 事件。
- RabbitMQ 用途：只承接 `echomind.chat.requests` 异步聊天请求、`echomind.chat.stream-events` SSE 事件、`echomind.chat-memory.persist.exchange` 普通聊天记忆分片写入，以及 `echomind.user-memory.requests` 用户长期记忆事件；Agent Team 当前不走 RabbitMQ。
...
flowchart TB
    ...
    subgraph DataAndInfra["数据与基础设施"]
        ...
        Rabbit["RabbitMQ<br/>chat.requests / chat.stream-events<br/>chat-memory shards / user-memory"]
