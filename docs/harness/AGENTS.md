...
- 记忆系统：MySQL 按 `userId + sessionId` 保存普通聊天完整会话；Redis 按字符预算保存单会话短期上下文；Redis Stack 保存用户长期事实向量；Milvus 保存 Agent 知识库切片正文和向量；Redis Hash 保存用户画像快照。
- Skill 系统：本地 JAR 插件通过 `SkillRegistry` 加载，启用后同步到 `CapabilityRegistry`。
- 工具路由：`ToolRouter` 是入口，具体策略拆在 `ToolCompatibilityPolicy`、`ToolMatchScorer`、`ToolDisambiguationPolicy` 和 `ToolParameterExtractor`。
...
7. 工具调用从 `CapabilityRegistry`、`SkillCapabilityService`、`ExternalMcpRuntimeService` 读。
   如果在看工具选择问题，继续读 `ToolRouter`、`ToolCompatibilityPolicy`、`ToolMatchScorer`、
   `ToolDisambiguationPolicy` 和 `ToolParameterExtractor`。
8. 前端从 `echomind-web/src/stores` 和页面组件读，先看状态如何跨路由保持。

## 硬约束

- MySQL 是 Agent、Skill 状态、前端完整会话历史、知识库元数据的事实来源。
- Redis 是 LLM 最近上下文来源；Redis Stack 是用户长期事实来源；Milvus 是知识库切片正文和向量事实来源；Redis 用户画像快照是事实层的压缩结果；LLM 不从 MySQL 读取聊天历史或知识库 chunk。
...
- Provider 不允许按具体 Skill 名称硬编码默认参数、工具选择或最终答案策略；新增工具应通过
  `keywords`、`aliases`、`host:` / `domain:` / `url-host:`、`direct-result` 等 metadata 自描述。
...
- 发现工具路由继续长硬编码时，优先把规则放进现有路由策略模块或让工具 metadata 自描述，不要把具体业务工具名写进 Provider。
