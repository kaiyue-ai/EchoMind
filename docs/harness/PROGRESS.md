...
- LLM Provider 已移除按具体工具名判断最终答案策略的硬编码；工具输出可直接交付时由 metadata tag `direct-result` 或 `final-answer` 声明。
- 工具路由已拆成更清晰的运行时模块：`ToolRouter` 保持入口，URL/domain 兼容性在 `ToolCompatibilityPolicy`，关键词评分在 `ToolMatchScorer`，铁路/日期等确定性消歧在 `ToolDisambiguationPolicy`，直调参数兜底在 `ToolParameterExtractor`。
...
- README、CLAUDE、AGENTS 和 harness 文档已同步项目详解、运行步骤、工具路由边界和验证命令。
