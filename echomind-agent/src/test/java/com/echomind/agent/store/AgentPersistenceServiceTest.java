 ...
 * Agent持久化服务单元测试。
 *
 * <p>这些测试不依赖真实MySQL，而是用Mock Mapper验证转换逻辑。
...
class AgentPersistenceServiceTest {
    ...
    void saveSerializesSkillIdsIntoEntity() {
        ...
        when(mapper.selectOptionalById("agent-a")).thenReturn(Optional.empty());
        AgentPersistenceService service = new AgentPersistenceService(mapper);
        ...
        config.setSkillIds(List.of("calculator", "date-query"));

        service.save(config);

        ArgumentCaptor<AgentEntity> captor = ArgumentCaptor.forClass(AgentEntity.class);
        verify(mapper).upsertById(captor.capture());
        ...
        assertThat(saved.getSkillIdsJson()).isEqualTo("[\"calculator\",\"date-query\"]");
    }
