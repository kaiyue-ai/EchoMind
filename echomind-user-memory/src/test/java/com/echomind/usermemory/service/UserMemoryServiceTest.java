...
class UserMemoryServiceTest {
    ...
    void skipsWhenDecisionDisablesBothFactAndProfile() {
        ...
        verify(fixture.analyzer(), never()).analyzeEpisodes(anyString(), anyString(), any(), any());
    ...
    void processesOneTurnImmediatelyAndWritesFactsAndProfileSnapshot() {
        ...
        when(fixture.analyzer().analyzeEpisodes(eq("user:user-a"), eq("旧画像"), any(), any())).thenReturn(
        ...
        when(fixture.analyzer().refreshProfile(eq("user-a"), eq("旧画像"), any())).thenReturn("用户维护 EchoMind，偏好中文注释。");
    ...
    void refreshesOnlyProfileWhenFactDecisionIsFalse() {
        Fixture fixture = fixture();
        when(fixture.analyzer().refreshProfile(eq("user-a"), eq(""), any())).thenReturn("新画像");
    ...
    void analyzesOnlyUserMessagesSoAssistantHallucinationsCannotBecomeEvidence() {
        ...
        when(fixture.analyzer().analyzeEpisodes(eq("user:user-a"), eq(""), any(), any())).thenReturn(
        ...
        verify(fixture.analyzer()).analyzeEpisodes(eq("user:user-a"), eq(""), turnCaptor.capture(), any());
