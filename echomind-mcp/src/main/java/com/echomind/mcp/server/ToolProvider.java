...
/**
 * 工具提供者接口 —— MCP Server 的核心 SPI（服务提供者接口）。
 *
 * <p>任何希望向 MCP Server 注册工具的实现都必须实现此接口。
 * 每个 ToolProvider 管理一个或多个相关的工具，例如：
 * <ul>
 ...
 * <p>设计决策：
 * <ul>
 *   <li>接口式设计 —— 将工具定义与 MCP 传输层解耦，便于单元测试和本地调用。</li>
 *   <li>批量注册 —— 一个 ToolProvider 可提供多个工具，
 *       减少注册代码量和 Provider 实例数量。</li>
 *   <li>与 Skill 接口互补 —— ToolProvider 是 MCP 原生的工具抽象，
 *       {@link com.echomind.mcp.client.MCPToolAdapter} 将其桥接到
 *       平台的 {@link com.echomind.skill.api.Skill} 生态。</li>
 * </ul>
 *
...
public interface ToolProvider {
     ...
     * @return 工具执行结果（成功或错误）
     */
    ToolResult call(String toolName, java.util.Map<String, Object> arguments);
}
